...
주어진 주제에 대해, 구글 검색 결과 상위 노출을 목표로 하는 블로그 게시물을 HTML 형식으로 작성해주세요.
...
3.  **본문 (Body - <h2>, <h3>, <p>, <strong>, <ul>, <li>, <table>):**
    *   논리적인 구조를 위해 여러 개의 소제목(<h2>)으로 내용을 나누어주세요.
    *   각 섹션은 주제에 대한 깊이 있는 정보를 제공해야 합니다. 단순한 사실 나열이 아닌, '왜'와 '어떻게'를 설명해주세요.
    *   정보 비교, 데이터 요약 등 표로 표현하면 더 명확하고 보기 좋은 내용은 적극적으로 <table> 태그를 사용하여 표로 만들어주세요.
    *   독자와의 공감대를 형성하고 신뢰를 줄 수 있는 내용으로 작성해주세요.
...
4.  **FAQ 섹션 (Frequently Asked Questions):**
    ...
    *   질문과 답변은 **Q1, A1, Q2, A2...** 와 같이 순서대로 번호를 매겨서 작성해주세요. 각 질문은 <strong>태그로 강조해주세요. (예: <p><strong>Q1. 질문 내용</strong></p><p>A1. 답변 내용</p>)
...
6.  **태그 (Tags):**
    *   게시물 내용과 관련된 핵심 키워드 및 LSI 키워드 10개를 쉼표(,)로 구분하여 나열해주세요. (예: 키워드1, 키워드2, 키워드3)
...
**--- 출력 규칙 ---**
*   위 모든 지침을 따라 블로그 본문에 들어갈 HTML 콘텐츠만 작성해주세요.
*   출력은 \`<h1>\` 태그로 바로 시작해야 합니다.
`;
